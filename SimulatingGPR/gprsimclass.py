...
class gprsim:
    def __init__(self, eps_r: float, rf: float, dt: float, dx: float, reflectors: list, region_shape: tuple, wavetype: str, SNR: float):
        ...
        self.SNR = SNR
        self.data, self.x_positions, self.t_samples= None, None, None
        ...
        self.data, self.x_positions, self.t_samples = data.T, x_positions, t_samples
        


    def NMO_correction(self, t_0, x_0):
        """
        ...
        """
        v = (c/np.sqrt(self.eps_r))/1e9         # [ns]
        
        NMO_corrected_data = np.zeros_like(data)
        nt, nx = self.data.shape
        for x in range(nx):
            x_offset = np.abs((x*self.dx) - x_0)
            t_correction = (np.sqrt(t_0**2+(2*x_offset/v)**2)-t_0)/(self.dt/1e-9)
            trace_original = slef.data[:, x]
    ...
    def minimize_tracewise_slope_window(self, n_candidates, window):
        """
        ...
        x = np.arange(n_traces) * self.dx
    
        # Precompute top-n candidate indices for each column
        sorted_indices = np.argsort(self.data, axis=0)[::-1, :]
        ...
        for i in range(2, n_traces):
            ...
            local_trend_mean = np.mean(local_trends) / self.dx
    
            trace_candidates = candidates[i]
            slope_diff = ((trace_candidates - path[i-1]) / self.dx) - local_trend_mean
    ...
    def smooth_path_dp(self, lam):
        ...
        """
        nt, nx = self.data.shape
        C = np.zeros_like(self.data)
        backtrack = np.zeros_like(self.data, dtype=int)
        ...
        for x in range(1, nx):
            for t in range(nt):
                ...
                C[t, x] = self.data[t, x] + penalties[best_prev]
    ...
    def visualize_preconditioning():
        ...
        for SNR, i in zip([0.1, 0.05, 0.01], range(0,3)):
            ...
            new = gprsim(eps_r, rf, dt, dx, reflectors, region_shape, wavetype, SNR)
            data, x_positions, t_samples = new.data, new.x_positions, new.t_samples
    ...
    def fit_hyperbola(self, num_hyperbolas, method):
        ...
        if method == 'fit_from_max': 
            ...
            t_points = self.data.argmax(axis=0) * self.dt # [ns] --> [s]
            x_points = np.arange(0, self.data.shape[1]) * self.dx # m
        ...
        if method=='robust_fit':
            ...
            x, t, c, b = smooth_path_dp(self.data, self.dx, lam=0.001)
            ...
            if alpha < 0 or (gamma - beta**2) < 0:
                ...
                return fit_hyperbola(c, num_hyperbolas, 'fit_from_max', self.dx, self.dt)
            else:
    ...
    def minimize_tracewise_slope_greedy(self, n_candidates, dx):
        t = self.data.argsort(axis=0)[::-1,:][0]
        t_og = self.data.argsort(axis=0)[::-1,:][0]* -1
        x = np.arange(0, self.data.shape[1]) * self.dx # 
        t_nmax = self.data.argsort(axis=0)[::-1,:] * -1
    
        candidates = np.array([self.data.argsort(axis=0)[::-1,:][n] for n in range(0,n_candidates)])
